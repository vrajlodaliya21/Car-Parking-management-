...
if (isset($_GET['action'])) {
    if ($_GET['action'] === 'get_areas' && isset($_GET['city'])) {
        $city = mysqli_real_escape_string($conn, $_GET['city']);

        // Get areas for the selected city
        $area_query = "SELECT DISTINCT area FROM vendors WHERE city = ? ORDER BY area";
        $stmt = mysqli_prepare($conn, $area_query);
    ...
    } elseif ($_GET['action'] === 'get_locations' && isset($_GET['city']) && isset($_GET['area'])) {
        $city = mysqli_real_escape_string($conn, $_GET['city']);
        $area = mysqli_real_escape_string($conn, $_GET['area']);

        $location_query = "SELECT DISTINCT location FROM vendors WHERE city = ? AND area = ? ORDER BY location";
        $stmt = mysqli_prepare($conn, $location_query);
        mysqli_stmt_bind_param($stmt, 'ss', $city, $area);
        mysqli_stmt_execute($stmt);
...
<body>
    ...
    <script>
        ...
        document.getElementById('city').addEventListener('change', function () {
            ...
            if (city) {
                fetch(`Login.php?action=get_areas&city=${encodeURIComponent(city)}`)
        ...
        document.getElementById('area').addEventListener('change', function () {
            ...
            if (city && area) {
                fetch(`Login.php?action=get_locations&city=${encodeURIComponent(city)}&area=${encodeURIComponent(area)}`)
                    .then(response => response.json())
                    .then(data => {
